...
def get_user_history(
    ...
    """
    history_records = (
        db.query(history_models.AnalysisHistory)
...
def create_history(
    ...
    match_score = calculate_match_score(
        history_in.have_skills, history_in.missing_skills
    )

    db_history = history_models.AnalysisHistory(
        user_id=current_user.id, match_score=match_score, **history_in.model_dump()
    )
    db.add(db_history)
    db.commit()
...
def update_history(
    ...
    db_history = (
        db.query(history_models.AnalysisHistory)
        .filter(history_models.AnalysisHistory.id == history_id)
        .first()
    )
