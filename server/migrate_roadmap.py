...
def migrate():
    ...
    with engine.connect() as conn:
        try:
            # For PostgreSQL
            conn.execute(text('ALTER TABLE users ADD COLUMN IF NOT EXISTS roadmap JSONB;'))
            conn.execute(text('ALTER TABLE analysis_history ADD COLUMN IF NOT EXISTS jd_text TEXT;'))
            conn.commit()
            ...
            try:
                ...
                conn.execute(text('ALTER TABLE analysis_history ADD COLUMN jd_text TEXT;'))
                conn.commit()
