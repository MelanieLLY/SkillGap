        ...
        <div className="glass-card-accent p-6">
            ...
            {!isLoading && !roadmap && !error && (
                <div className="text-center py-8 text-[#5f6573]">
                    <svg className="w-10 h-10 mx-auto mb-3 opacity-40" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            {!isLoading && roadmap && (
                <div className="space-y-6">
                    ...
                    <div className="flex flex-wrap gap-3 text-xs">
                        <SummaryBadge
                            icon="📅"
                            label={`${roadmap.total_estimated_duration_weeks} weeks`}
                        />
                        <SummaryBadge
                            icon="📚"
                            label={`${roadmap.summary.total_courses} courses`}
                        />
                        <SummaryBadge
                            icon="🛠"
                            label={`${roadmap.summary.total_projects} projects`}
                        />
                        <SummaryBadge
                            icon="⏱"
                            label={`${roadmap.summary.total_learning_hours}h total`}
                        />
                        <SummaryBadge
                            icon="🎯"
                            label={roadmap.summary.recommended_weekly_pace}
                        />
                    </div>
                                    ...
                                    <div key={phase.phase} className="relative">
                                        ...
                                        >
                                            ...
                                            <div className="z-10 w-9 h-9 rounded-full bg-[#1a1f2e] border-2 border-[#38e5b1]/40 flex items-center justify-center flex-shrink-0 group-hover:border-[#38e5b1] transition-colors">
                                                <span className="text-xs font-bold text-[#38e5b1]">
                                                    {phase.phase}
                                                </span>
                                            </div>
                                            ...
                                            <div className="flex-grow flex items-center justify-between bg-[#1a1f2e] hover:bg-[#1f2538] border border-white/5 rounded-xl px-5 py-3.5 transition-all group-hover:border-[#38e5b1]/20">
                                                <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                                                    ...
                                                    <span className="text-xs text-[#9aa0ac]">
                                                        Weeks {phase.start_week}–{phase.end_week} · {phase.weekly_commitment_hours}h/week
                                                    </span>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    {/* Skill pills (compact) */}
                                                        ...
                                                        >
                                                            ...
                                                            <ul className="space-y-1">
                                                                {project.deliverables.map((d, di) => (
                                                                    <li key={di} className="flex items-start gap-2 text-xs text-[#9aa0ac]">
