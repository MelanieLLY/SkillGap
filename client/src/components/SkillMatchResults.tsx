...
const SkillMatchResults: React.FC<SkillMatchResultsProps> = ({ skills }) => {
  ...
  const SkillPill: React.FC<{ skill: string; variant: "have" | "missing" | "bonus" }> = ({
    ...
    return (
      <div
        className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm font-medium ${styles[variant]}`}
      >
        ...
        <div className="flex-grow grid grid-cols-3 gap-4 w-full">
          ...
          <div>
            ...
            <div className="space-y-2">
              {hasResults ? (
                have.length > 0 ? (
                  have.map((skill, i) => <SkillPill key={i} skill={skill} variant="have" />)
                ) : (
                  <p className="text-xs text-[#5f6573] italic text-center py-4">None found</p>
                )
              ) : (
                <p className="text-xs text-[#5f6573] italic text-center py-4">
                  Paste a JD to begin
                </p>
              )}
            </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {hasResults ? (
                missing.length > 0 ? (
                  missing.map((skill, i) => <SkillPill key={i} skill={skill} variant="missing" />)
                ) : (
                  <p className="text-xs text-[#5f6573] italic text-center py-4">
                    Perfect match! 🎉
                  </p>
          ...
          <div>
            ...
            <div className="space-y-2">
              {hasResults ? (
                bonus.length > 0 ? (
                  bonus.map((skill, i) => <SkillPill key={i} skill={skill} variant="bonus" />)
                ) : (
                  <p className="text-xs text-[#5f6573] italic text-center py-4">—</p>
                )
              ) : (
