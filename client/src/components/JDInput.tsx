...
const JDInput: React.FC<JDInputProps> = ({ onAnalyze, isLoading }) => {
    ...
    const handleSubmit = () => {
        if (text.trim().length === 0 || companyName.trim().length === 0 || positionName.trim().length === 0) return;
        ...
        <div className="glass-card-accent p-6 flex flex-col h-full">
            ...
            <div className="mt-5 flex flex-col gap-3 p-4 bg-[#161a25] border border-white/10 rounded-xl">
                ...
                <button
                    ...
                    disabled={isLoading || text.trim().length === 0 || companyName.trim().length === 0 || positionName.trim().length === 0}
                >
                    {isLoading ? (
                        <>
