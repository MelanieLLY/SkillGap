        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {history.map((record) => (
              <div
                key={record.id}
                className="glass-card-accent p-6 flex flex-col rounded-2xl border border-white/5 hover:border-[#38e5b1]/30 transition-colors"
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div
                    className={`shrink-0 flex items-center justify-center w-12 h-12 rounded-full border-2 ${
                      record.match_score >= 70
                        ? "border-green-500/50 text-green-400 bg-green-500/10"
                        : record.match_score >= 40
